...
import qualified Streamly.Internal.Data.ParserK as PRK
...
anyK value = IP.parseK (PRK.any (> value))
...
{-# INLINE manyK #-}
manyK :: MonadCatch m => SerialT m Int -> m Int
manyK xs = do
    x <- IP.parseK (AP.many (PRK.satisfy (> 0))) xs
...
{-# INLINE splitAllAnyK #-}
splitAllAnyK :: MonadCatch m
    => Int -> SerialT m Int -> m (Bool, Bool)
splitAllAnyK value =
    IP.parseK ((,) <$> PRK.any (>= (value `div` 2)) <*> PRK.any (> value))
...
{-# INLINE sequenceAK #-}
sequenceAK :: MonadCatch m => Int -> SerialT m Int -> m Int
sequenceAK value xs = do
    x <- IP.parseK (TR.sequenceA (Prelude.replicate value (PRK.satisfy (> 0)))) xs
    return $ Prelude.length x
...
{-# INLINE choiceK #-}
choiceK :: MonadCatch m => Int -> SerialT m Int -> m Int
choiceK value = do
    IP.parseK (PRK.choice (replicate value (PRK.satisfy (< 0)))
                AP.<|> PRK.satisfy (> 0))
...
o_1_space_serial_parse value =
    ...
    , benchIOSink value "anyK" $ anyK value
    , benchIOSink value "split (allK,anyK)" $ splitAllAnyK value
    , benchIOSink value "sequenceAK" $ sequenceAK value
    , benchIOSink value "manyK" $ manyK
    , benchIOSink value "choiceK" $ choiceK value
    ]
